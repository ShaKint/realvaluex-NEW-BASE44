...
import { Brain, BarChart3, ScanSearch, Star, TrendingDown, Zap, Settings, Shield, Menu, X, LogOut, User, Bell, Newspaper, Calculator } from 'lucide-react';
...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className={`min-h-screen bg-slate-950 text-white flex ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
      <aside className={`fixed inset-y-0 ${isRTL ? 'right-0' : 'left-0'} z-40 w-64 bg-slate-900 border-${isRTL ? 'l' : 'r'} border-white/5 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : (isRTL ? 'translate-x-full' : '-translate-x-full')} lg:relative lg:translate-x-0`}>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {navItems.map(({ key, icon: Icon, path }) => {
            ...
            return (
              <a key={key} href={path} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${active ? 'bg-indigo-600 text-white' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
          ...
          {isAdmin && (
            <>
              ...
              <a href="/admin" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-amber-400/70 hover:text-amber-400 hover:bg-amber-400/5 transition-all">
        ...
        <div className="p-3 border-t border-white/5 space-y-1">
          ...
          <button onClick={() => logout()} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-white/30 hover:text-white/70 hover:bg-white/5 transition-all">
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-auto">
        <header className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-white/5 bg-slate-950/80 backdrop-blur sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden w-8 h-8 flex items-center justify-center rounded-lg text-white/60 hover:text-white hover:bg-white/5">
